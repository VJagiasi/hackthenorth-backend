...
const scanFriend = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const { badge_code } = req.params; // Scanner's badge_code
        const { friend_badge_code } = req.body; // Friend's badge_code
        // ✅ Validate input
        if (!friend_badge_code || typeof friend_badge_code !== "string") {
            return res.status(400).json({ error: "Invalid or missing friend_badge_code." });
        }
        // ✅ Prevent scanning your own badge
        if (badge_code === friend_badge_code) {
            return res.status(400).json({ error: "You cannot scan your own badge." });
        }
        ...
        if (!scanner.checked_in || !scanned.checked_in) {
            return res.status(403).json({ error: "Both users must be checked in to scan friends." });
        }
        ...
        if (existingScan) {
            return res.status(400).json({ error: "You have already scanned this friend." });
        }
