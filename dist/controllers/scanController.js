...
const addScan = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        if (!activity_name || typeof activity_name !== "string" || activity_name.trim().length === 0) {
            return res.status(400).json({ error: "Invalid activity_name provided." });
        }
        if (!activity_category || typeof activity_category !== "string" || activity_category.trim().length === 0) {
            return res.status(400).json({ error: "Invalid activity_category provided." });
        }
        // ✅ 2. Find user by badge_code
        const user = yield prisma_1.default.user.findUnique({ where: { badge_code } });
        if (!user)
            return res.status(404).json({ error: "User not found" });
        // ✅ 3. Prevent scanning if user is checked out
        if (!user.checked_in) {
            return res.status(403).json({ error: "User is checked out. Please check in before scanning." });
        }
        // ✅ 4. Find activity (case insensitive)
        const activity = yield prisma_1.default.activity.findFirst({
            where: { name: activity_name.toLowerCase() },
        });
        if (!activity) {
            return res.status(400).json({ error: `Activity '${activity_name}' does not exist.` });
        }
        // ✅ 5. Ensure category matches
        ...
        // ✅ 6. Prevent multiple scans for "one-scan-only" activities
        if (activity.one_scan_only) {
            ...
            if (existingScan) {
                return res.status(400).json({ error: `You are only allowed one scan for '${activity_name}'.` });
            }
        }
        // ✅ 7. Prevent spam scanning (5-second cooldown)
        ...
        if (lastScan && new Date().getTime() - new Date(lastScan.scanned_at).getTime() < SCAN_COOLDOWN_MS) {
            return res.status(429).json({ error: "You are scanning too fast. Please wait a few seconds." });
        }
        ...
        // ✅ 9. Update `updated_at` for the user
...
const getScansData = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        if (min_frequency && isNaN(Number(min_frequency)) || max_frequency && isNaN(Number(max_frequency))) {
            return res.status(400).json({ error: "min_frequency and max_frequency must be valid numbers." });
        }
        // ✅ Get activity IDs first to filter scans
        ...
        // Filter results after query based on frequency
        const filteredScanData = scanData.filter((scan) => scan._count.id >= min && scan._count.id <= max);
        if (filteredScanData.length === 0) {
            return res.status(404).json({ error: "No scans found matching the criteria." });
        }
...
const getScansByTime = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        // ✅ 1. Validate `activity_name`
        if (!activity_name || typeof activity_name !== "string" || activity_name.trim().length === 0) {
            return res.status(400).json({ error: "Valid activity_name is required." });
        }
        // ✅ 2. Validate time interval
        const validIntervals = ["hour", "minute", "day"];
        if (!validIntervals.includes(interval)) {
            return res.status(400).json({ error: "Invalid interval. Use 'hour', 'minute', or 'day'." });
        }
        // ✅ 3. Convert timestamps to Date objects (if provided)
        ...
        // ✅ 4. Get activity ID (case-insensitive search)
        const activity = yield prisma_1.default.activity.findFirst({
        ...
        // ✅ 5. Define SQL-friendly `DATE_TRUNC()` clause
        const interval_value = interval;
        // ✅ 6. Prepare and execute parameterized query
    ...
    `));
        if (query.length === 0) {
            return res.status(404).json({ message: "No scan data found for this activity within the given time range." });
        }
        // ✅ 7. Format results properly
